...
async def MountTracking(expId, butler, client):
    ...
    """Queries the EFD for a given exposure and checks if there were tracking errors
        ...
        Returns
            ...
            """
    expString = str(expId)
    dayObs = expString[0:4] + '-' + expString[4:6] + '-' + expString[6:8]
    seqNum = expString[10:13]
    ...
    if (imgType not in ['OBJECT', 'SKYEXP', 'ENGTEST']) or (exptime < 1.99):
        return 
    end = time.time()
    elapsed = end-start
    ...
    plt.savefig('/project/cslage/AuxTel/mount_graphs/AuxMntTorques_dayObs_%s_seqNum_%s.png'%(dayObs, seqNum))
    plt.close()
...
async def main():
    ...
    parser.add_argument('--expId', help='expId: Format 2021021800736; If expId == All, then look for a dayObs and run all expIds from that dayObs')
    parser.add_argument('--dayObs', '-o', help='dayObs: Format = 2021-06-08')
    ...
    expId = args.expId
    if expId == 'All':
        intDayObs = int(args.dayObs.replace('-', ''))
        exposureList = []
        for record in butler.registry.queryDimensionRecords("exposure", where="exposure.day_obs=%d"%intDayObs):
            exposureList.append([record.id, record])
        exposureList.sort(key=lambda x: x[0])
        for [expId,record] in exposureList:
            try:
                await MountTracking(expId, butler, client)
            except:
                continue
    else:
        expId = int(expId)
