...
async def MountTracking(expId, pdf, butler, client, fail_limit=1.0, makeGraph=True):
    start = time.time()
...
Start = {tStart}, End = {tEnd}", fontsize = 18)
    ...
    if makeGraph:
        return (expId, az_rms, el_rms, rot_rms)
    else:
        return None
...
async def main():
    ...
    parser.add_argument('--dayObs', help='dayObs: Format 2021-03-11')
    parser.add_argument('--fail_limit', help='fail_limit: Format float')
    args = parser.parse_args()
    dayObs = args.dayObs
    fail_limit = float(args.fail_limit)

    client = EfdClient('summit_efd')
    REPO_DIR = '/readonly/lsstdata/auxtel/base/auxtel/oods/butler/repo'
    butler = Butler(REPO_DIR)
    visits = butler.queryMetadata('raw', ['expId'], detector=0, dayObs=dayObs)
    fail_string = str(fail_limit).replace('.', 'p')
    pdf = PdfPages('/home/craiglagegit/DATA/mount_graphs/tracking_errors/Tracking_%s_%s.pdf'%(fail_string,dayObs))
    fails = []
    for expId in visits[110:]:
        try:
            result = await MountTracking(expId, pdf, butler, client, fail_limit=fail_limit)
            if result:
                fails.append(result)
        except:
            continue
    pdf.close()
    for fail in fails:
        (expId, az_rms, el_rms, rot_rms) = fail
        print(f"{expId} failed! az_rms = {az_rms}, el_rms = {el_rms}, rot_rms = {rot_rms}")
    return 0
