...
async def MountTracking(expId, pdf, butler, client, fail_limit=0.5, makeGraph=True):
    ...
    if (imgType not in ['OBJECT', 'SKYEXP', 'ENGTEST', 'DARK', 'BIAS']):
        return True
    end = time.time()
    elapsed = end-start
    ...
    # Get the data
    t_start = Time(tStart, scale='tai') - TimeDelta(1, format='sec')
    t_end = Time(tEnd, scale='tai') + TimeDelta(1, format='sec')
    ...
    rot_torque_vals = np.array(rot_torque.values.tolist())[:,0]
    times = np.array(az.values.tolist())[:,1]
    times = times - times[0]

    rot_mean = np.mean(rot_vals)
    rot_torque_mean = np.mean(rot_torque_vals)
    ...
    if (az_rms > fail_limit) or (el_rms > fail_limit) or (rot_rms > fail_limit):
        ...
        return (rot_mean, rot_torque_mean)

### MAIN FUNCTION ###
async def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--firstExpId', help='firstExpId: Format 2021021800736')
    parser.add_argument('--lastExpId', help='lastExpId: Format 2021021800736')
    args = parser.parse_args()
    firstExpId = int(args.firstExpId)
    lastExpId = int(args.lastExpId)

    client = EfdClient('summit_efd')
    REPO_DIR = '/readonly/lsstdata/auxtel/base/auxtel/oods/butler/repo'
    butler = Butler(REPO_DIR)
    pdf = PdfPages('/home/craiglagegit/DATA/mount_graphs/Tracking_Tests_Rotator_08Mar21.pdf')
    ...
    for expId in range(firstExpId, lastExpId+1):
        try:
            ...
            rot_vals.append(result[0])
            rot_torque_vals.append(result[1])
        except:
            continue
    pdf.close()

    plt.figure(figsize=(8,8))
    plt.title("Rotator Torque vs Rotator Angle")
    plt.scatter(rot_vals, rot_torque_vals)
    plt.xlabel("ROTPA(degrees)")
    #plt.ylim(0.0, 0.20)
    #plt.legend()
    plt.ylabel("Rotator Torque")
    plt.savefig("/home/craiglagegit/DATA/mount_graphs/Rotator_Torque_08Mar21.pdf")
