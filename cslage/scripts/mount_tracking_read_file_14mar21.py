...
async def MountTracking(expId, butler, client, fail_limit=1.0, makeGraph=True):
    start = time.time()
    ...
    if (imgType not in ['OBJECT', 'SKYEXP', 'ENGTEST']):
        return None
    end = time.time()
    elapsed = end-start
    ...
    #if (az_rms > fail_limit) or (el_rms > fail_limit) or (rot_rms > 1000.0 * fail_limit):
    #    makeGraph = True
    #else:
    #    makeGraph = False
...
Start = {tStart}, End = {tEnd}", fontsize = 18)
        # Azimuth axis
        plt.subplot(3,3,1)
    ...
    #if makeGraph:
    return (expId, azimuth, elevation, az_rms, el_rms, rot_rms)
    #else:
    #    return None
...
async def main():
    ...
    outFile = '/home/craiglagegit/DATA/mount_graphs/tracking_errors/Fails_on_Sky.pdf'
    inFile = '/home/craiglagegit/DATA/mount_graphs/tracking_errors/fails_0p5.txt'
    inFails = open(inFile, 'r')
    lines = inFails.readlines()
    inFails.close()
    
    fails = []
    for line in lines:
        #try:
        expId = int(line.split(' ')[0])
        print(expId)
        result = await MountTracking(expId, butler, client, fail_limit=0.5, makeGraph=False)
        if result:
            fails.append(result)
        #except:
        #    continue
    ...
    el_fails_el = []
    for fail in fails:
        (expId, azimuth, elevation, az_rms, el_rms, rot_rms) = fail
        if azimuth < 0.0:
            azimuth += 360.0
        if az_rms > 0.5:
            az_fails_az.append(azimuth)
            az_fails_el.append(elevation)
        if el_rms > 0.5:
            el_fails_az.append(azimuth)
            el_fails_el.append(elevation)

    fig = plt.figure(figsize = (16,8))
    plt.suptitle("Mount Tracking Locations", fontsize = 24)
    plt.scatter(az_fails_az, az_fails_el, marker = 'x', s=200, color = 'red', label = 'Azimuth fails')
    plt.scatter(el_fails_az, el_fails_el, marker = '+', s=200, color = 'blue', label = 'Elevation fails')
    plt.xlim(0, 360)
    plt.ylim(0, 90)
    plt.xlabel("Azimuth(degrees)", fontsize=24)
    plt.ylabel("Elevation(degrees)", fontsize=24)
