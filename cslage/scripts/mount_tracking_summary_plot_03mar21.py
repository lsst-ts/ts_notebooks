...
async def MountTracking(expId, pdf, butler, client, fail_limit=0.5, makeGraph=True):
    ...
    start = time.time()
    return (azimuth, elevation, az_rms, el_rms, rot_rms)
    """
...
async def main():
    ...
    pdf = PdfPages('/home/craiglagegit/DATA/mount_graphs/Tracking_Tests_03Mar21.pdf')
    ...
    plt.title("RMS Elevation tracking Errors vs Azimuth")
    plt.scatter(az1, el1, label = "Elevation <= 40")
    plt.scatter(az2, el2, label = "Elevation > 40 and <= 60")
    plt.scatter(az3, el3, label = "Elevation > 60")
    plt.xlabel("Azimuth(degrees)")
    plt.ylim(0.0, 0.20)
    plt.legend()
    plt.ylabel("Elevation RMS error (arcseconds)")
    plt.savefig("/home/craiglagegit/DATA/mount_graphs/Elevation_Errors_03Mar21.pdf")
    return 0
